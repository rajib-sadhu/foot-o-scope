...
export const createBlogs = async (req:Request,res:Response):Promise<Response> => {
    try {
        const {blogTitle,blogDesc,blogImageName,blogTagName} = req.body;
        const addBlog:QueryResult = await pool.query("INSERT INTO tbl_blog (blog_title,blog_desc,blog_image_file_name,blog_tag_name) VALUES ($1,$2,$3,$4)",[blogTitle,blogDesc,blogImageName,blogTagName])
    return res.status(200).json({message:"user created successfully", body:{BlogInfo:{blogTitle,blogDesc,blogImageName,blogTagName}}});
    } catch (error) {
